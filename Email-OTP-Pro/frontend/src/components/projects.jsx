...
export default function Projects({ projects: initialProjects, onAddProject }) {
    ...
    const handleAddProject = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/projects/new-project', {
                method: 'POST',
    ...
    const handleSaveChanges = async () => {
        try {
            const response = await fetch(`http://localhost:5001/api/projects/${selectedProject._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    status: manageStatus,
